...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', verbose=False):
        '''
    ...
    def evaluate(self, id_setup):
        ...
        '''
        tic = time.time()
        if self.verbose: print('Running per image evaluation...')
        p = self.params
        # add backward compatibility if useSegm is specified in params
        if not p.useSegm is None:
            p.iouType = 'segm' if p.useSegm == 1 else 'bbox'
            print('useSegm (deprecated) is not None. Running {} evaluation'.format(p.iouType))
        if self.verbose: print('Evaluate annotation type *{}*'.format(p.iouType))
